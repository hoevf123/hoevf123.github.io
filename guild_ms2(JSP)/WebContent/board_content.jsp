	     ...
	     <div class="info-buliten-board div-content">
		     ...
		     <%
		     	try{
			     	...
			     	ResultSet rs = boardDAO.select("SELECT page_title, page_datas, page_author FROM board WHERE page_id=?",page_id);
			     	if(rs == null || rs.isBeforeFirst() == false){
			     		throw new NullPointerException("Not Found Resultset");
			     	}
			     	else{
			     		rs.next();
			     		String page_title = rs.getString(1);
				     	String page_datas = rs.getString(2);
				     	String page_author = rs.getString(3);
				     	%>
				     	<h1><%= page_title %></h1>
				     	<p class="author"><%= page_author %></p>
